...
switch ($post['accion']) {
    ...
    case 'unCarro':
        $sql = sprintf(
            ...
            mysqli_real_escape_string($conection, $post['cod_carro'])
        );
        $query = mysqli_query($conection, $sql);
    ...
    case 'pagar':
        $sql = sprintf(
            ...
            WHERE cod_carro='%s'",
            mysqli_real_escape_string($conection, $post['valor_pago']),
            mysqli_real_escape_string($conection, "Pagado"),
            mysqli_real_escape_string($conection, $post['cod_carro'])
        );
        $query = mysqli_query($conection, $sql);
    ...
    case 'insertar':
        $sql = sprintf(
            ...
            mysqli_real_escape_string($conection, $post['cedula_carro']),
            mysqli_real_escape_string($conection, $post['nombre_carro']),
            mysqli_real_escape_string($conection, $post['placa_carro']),
            mysqli_real_escape_string($conection, 'Pendiente')
        );
        $query = mysqli_query($conection, $sql);

        if (!$query) {
            $respuesta = ['code' => 400, 'response' => mysqli_error($conection), 'estado' => false];
        } else {
            $respuesta = $query
